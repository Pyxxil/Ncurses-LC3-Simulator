...
static int init_machine(struct program *program)
{
	...
	while (1 == (ret = populate_memory(program))) {
	...
	while (1) {
		...
		if (QUIT == input) {
			...
			*current_state = MAIN;
			...
			execute_next(&(program->simulator), output);
			program->simulator.isPaused = true;
			print_state(&(program->simulator), state);
		...
		if (!program->simulator.isPaused && !program->simulator.isHalted) {
			execute_next(&(program->simulator), out);
			timeout = 0;
		} else {
			set_state(current_state);
			print_state(&(program->simulator), state);
			timeout = -1;
			generate_context(context, program, 0, program->simulator.PC);
		}
	}
}
	...
	while (1) {
		...
		if (QUIT == input) {
			...
			*current_state = MAIN;
			...
			if (jump_address == selected_address)
				continue;
			generate_context(window, program, 0, (uint16_t) jump_address);
		} else if (KEYUP == input) {
			move_context(window, program, UP);
		} else if (KEYDOWN == input) {
			move_context(window, program, DOWN);
		} else if (EDITFILE == input) {
			new_value = popup_window(
				"Enter the new instruction (in hex): ",
				program->simulator.memory[selected_address].value,
				false);
			program->simulator.memory[selected_address].value =
				(uint16_t) new_value;
			update(window, program);
		} else if (SETPC == input) {
			program->simulator.PC = selected_address;
		} else if (BREAKPOINTSET == input) {
			program->simulator.memory[selected_address]
...
static void run_machine(struct program *program)
{
	...
	while (simulating) {
		...
		case MEM:
			if (-1 == mem_populated) {
				generate_context(context, program, 0,
						program->simulator.PC);
			} else {
				generate_context(context, program, selected,
...
void start_machine(struct program *program)
{
	initscr();
	atexit(exit_handle);
